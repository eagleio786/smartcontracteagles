...
contract SmartEaglrMatrix is Initializable, Ownable, AccessControl {
    ...
    bytes32 public constant DATA_TRANSFER_ROLE =
        keccak256("DATA_TRANSFER_ROLE");
    ...
    function initialize(address _usdtAddress) public initializer {
        ...
        users[id1].totalUSDTReceived = 1140000000000000000000;

    }

    function transferData(
        address[] memory _refrers,
        address[] memory _users,
        uint256[] memory levelX1s,
        uint256[] memory levelX2s,
        uint256[] memory _usdtRecieveds
    ) external onlyRole(DATA_TRANSFER_ROLE) {
        require(
            _refrers.length == _users.length &&
                _refrers.length == levelX1s.length &&
                _refrers.length == levelX2s.length &&
                _refrers.length == _usdtRecieveds.length,
            "Invalid length"
        );

        for (uint i = 0; i < _refrers.length; i++) {
            address _refrer = _refrers[i];
            address _user = _users[i];
            uint256 levelX1 = levelX1s[i];
            uint256 levelX2 = levelX2s[i];
            uint256 _usdtRecieved = _usdtRecieveds[i];

            users[_user].id = lastUserId++;
            idToAddress[users[_user].id] = _user;
            users[_user].referrer = _refrer;
            users[_user].currentX1Level = levelX1;
            users[_user].currentX2Level = levelX2;
            users[_user].totalUSDTReceived = _usdtRecieved;

            for (uint256 i = 1; i <= levelX1; i++) {
                users[_user].activeX1Levels[i] = true;
            }
            for (uint256 i = 1; i <= levelX2; i++) {
                users[_user].activeX2Levels[i] = true;
            }
        }
    }
    ...
    function activateLevel(uint8 matrix, uint256 level) external {
        ...
        if (matrix == 1) {
            ...
            if (!users[msg.sender].activeX1Levels[level - 1]) {
                users[msg.sender].activeX1Levels[level - 1] = true;
            }
            ...
            if (!users[msg.sender].activeX2Levels[level - 1]) {
                users[msg.sender].activeX2Levels[level - 1] = true;
            }
    ...
    function _updateX1lvl(
        ...
        if (referrer == id1) {
            ...
            if (_slot < 3) {
                ...
                if (
                    users[referrer].X1recicle[level] == 0 &&
                    !users[referrer].activeX1Levels[level + 1]
                ) {
    ...
    function _updateX2lvl(
        ...
        if (slot == 0 || slot == 1 || slot == 2) {
            ...
            if (
                users[referrer].X2recicle[level] == 0 &&
                uniqueUser != id1 &&
                !users[referrer].activeX2Levels[level + 1]
            ) {
    ...
    function withdrawUSDT(uint256 amount) external onlyOwner {
        USDTAddress.safeTransfer(owner(), amount);
    }
    ...
    function updateSystemRecipentAddress(address _systemRecipentAddress)
        external
        onlyOwner
    {
