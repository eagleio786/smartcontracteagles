...
async function main() {
  ...
  const provider = new ethers.JsonRpcProvider(process.env.RPC_URL_BNB_NETWORK);
  const signer = new ethers.Wallet(process.env.PRIVATE_KEY_PRODUCTION as string, provider);

  const contractAddress = process.env.CONTRACT_ADDRESS_BNB as string;
